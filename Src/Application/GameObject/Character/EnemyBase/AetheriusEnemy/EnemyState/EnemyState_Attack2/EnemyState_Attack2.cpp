...
#include"../EnemyState_Attack3/EnemyState_Attack3.h"
...
void EnemyState_Attack2::StateStart(RedEnemy* _owner)
{
	EnemyStateBase::StateStart(_owner);

	auto anime = _owner->GetAnimeModel()->GetAnimation("Attack2");
...
void EnemyState_Attack2::StateUpdate(RedEnemy* _owner)
{
	// 攻撃中の移動方向で回転を更新
	...
	{
		auto attack = std::make_shared<EnemyState_Attack3>();
...
void EnemyState_Attack2::StateEnd(RedEnemy* _owner)
{
	for (const auto& ref : m_enemyEffects)
	{
		if (auto effect = ref->GetEffectBase().lock())
...
void EnemyState_Attack2::ApplyFromConfig(const EnemyStateBase& other)
{
	assert(typeid(other) == typeid(EnemyState_Attack2));
	const auto& p = static_cast<const EnemyState_Attack2&>(other);
...
void EnemyState_Attack2::ExposeParametersImGui()
{
	ImGui::Text("Effects");
...
void EnemyState_Attack2::LoadParametersJson(const nlohmann::json& js)
{
	// 複数のみ
	if (js.contains("EnemyState_Attack2_Effects") && js["EnemyState_Attack2_Effects"].is_array())
	{
		m_enemyEffects.clear();
		for (const auto& node : js["EnemyState_Attack2_Effects"])
		{
	...
	if (!js.contains("EnemyState_Attack2")) return;
	const auto& stateNode = js["EnemyState_Attack2"];
...
void EnemyState_Attack2::SaveParametersJson(nlohmann::json& js) const
{
	// 複数のみ
	...
	js["EnemyState_Attack2_Effects"] = std::move(arr);

	if (!js.contains("EnemyState_Attack2")) js["EnemyState_Attack2"] = nlohmann::json::object();
	auto& stateNode = js["EnemyState_Attack2"];
