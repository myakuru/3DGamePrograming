...
public:
	...
	RedEnemy() { m_typeID = TypeID; AddTag(ObjTag::EnemyLike); }
	...
	// ステート遷移 // コンスと参照にする
	void ChangeState(const std::shared_ptr<EnemyStateBase>& _state);
	...
	StateManager<RedEnemy>& GetStateMachine() { return m_stateMachine; }
...
private:
	...
	StateManager<RedEnemy> m_stateMachine;
