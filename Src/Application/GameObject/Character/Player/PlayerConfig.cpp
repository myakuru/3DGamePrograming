...
// Hit
#include "Application/GameObject/Character/Player/PlayerState/PlayerState_HitBase/PlayerState_Hit/PlayerState_Hit.h"
...
{
	for (const auto& st : m_states) st->JsonInput(js);
}

void PlayerConfig::JsonSave() const
{
	nlohmann::json cfg = nlohmann::json::object();
	for (auto& st : m_states) st->JsonSave(cfg);
...
void PlayerConfig::CreateStates()
{
	...
	// m_states.emplace_back(std::make_unique<PlayerState_SpecialAttack1>()); m_stateNames.emplace_back("SpecialAttack1"); // 実体なしのため除外
	...
	// m_states.emplace_back(std::make_unique<PlayerState_ChargeAttackMax3>()); m_stateNames.emplace_back("ChargeAttackMax3"); // 実体なしのため除外

	// 回避
	m_states.emplace_back(std::make_unique<PlayerState_ForwardAvoid>());      m_stateNames.emplace_back("FowardAvoid");
	m_states.emplace_back(std::make_unique<PlayerState_ForwardAvoidFast>());  m_stateNames.emplace_back("FowardAvoidFast");
...
void PlayerConfig::ApplyPrototypeParametersTo(StateBase<Player>& _runtime)
{
	for (const auto& proto : m_states)
	{
		if (_runtime.TryApplyFromPrototype(*proto))
		{
			break;
		}
	}
